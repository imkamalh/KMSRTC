...
import com.dev.it.kmskedelainew.artikel.adapters.KomentarArtikelAdapter;
...
public class KomenArtikelActivity extends AppCompatActivity implements SwipeRefreshLayout.OnRefreshListener{
    private String idartikel;
    RecyclerView recyclerView;
    KomentarArtikelAdapter adapter;
    ProgressBar progressBar;
    CoordinatorLayout coordinatorLayout;
    private static final String STATE_MISSIONS = "state_komenartikel";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        idartikel = getIntent().getStringExtra("idartikel");
        ...
        adapter = new KomentarArtikelAdapter(getApplicationContext());
            ...
            public void onClick(View v) {
                if(!sessionManager.isLoggedIn()){
                    AlertDialog.Builder builder  = new AlertDialog.Builder(v.getRootView().getContext())
                            .setMessage("Please login or register to comment")
                ...
                map.put("artikel_id", idartikel);
                map.put("isi_komen", komen.getText().toString());

                CustomRequest customRequest = new CustomRequest(Request.Method.POST, URLEndpoints.SUBMIT_COMEENTS_ARTICLES, map, new Response.Listener<JSONObject>() {
                    @Override
                        ...
                        try{
                            ...
                            if (res.equals("success")){
                                listData.add(new Komentar(response.getString("tanggal"), komen.getText().toString(),
                                        sessionManager.getUserDetails().get(SessionManager.KEY_NAME), "aaa", Integer.valueOf(idartikel),response.getInt("id_komen")
    ...
    private void loadKomentar(){
        if (!StateKMS.isNetworkConnected(getApplicationContext())){
            ...
            JsonArrayRequest jsonArrayRequest = new JsonArrayRequest(URLEndpoints.GET_COMEENTS_ARTICLES + idartikel, new Response.Listener<JSONArray>() {
                @Override
                    ...
                    try{
                        for(int i = 0; i < response.length(); i++){
                            ...
                            listData.add(new Komentar(jsonObject.getString("tanggal_komen"),
                                    ...
                                    , "aaa", jsonObject.getInt("artikel_id")
